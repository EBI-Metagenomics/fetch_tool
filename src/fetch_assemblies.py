...
from src.exceptions import NoDataError, ENAFetch204
...
class FetchAssemblies(AbstractDataFetcher):
    ...
    #?accession=PRJEB52681&result=analysis&fields=study_accession,sample_accession,analysis_accession,analysis_type,tax_id,scientific_name,generated_ftp,submitted_ftp&format=json&download=False&limit=0"
    ...
    def _retrieve_project_info_from_api(self, project_accession):
        ...
        try:
            ...
            logging.info("The data portal API returned a 204, trying the filereport API")
            fetch_204_ex = ex
        
        # Alternative endpoint, the filereport which is the used on the ENA website
        if fetch_204_ex:
            file_report_url = self.ENA_FILEREPORT_URL + \
                "?" + "&".join([
                "&".join(self.ENA_PORTAL_PARAMS) + ",".join(self.ENA_PORTAL_FIELDS),
                f"accession={project_accession}",
            ])
            data = self._retrieve_ena_url(file_report_url)
            if not data:
                raise NoDataError("It was not possible to fetch data from the Portal API or the Filereport API")
    ...
    def _combine_analyses(metadata, wgs):
        ...
        for assembly in wgs:
            ...
            wgs_dict[analysis_id] = {
                "CONTIG_CNT": assembly["CONTIG_CNT"],
    ...
    def _retrieve_insdc_assemblies(self, study_accession):
        ...
        mapped_data = []
        json_data = self._retrieve_ena_url(
            self.ENA_WGS_SET_API_URL.format(study_accession)
        )
        logging.info(
            "Retrieved {count} assemblies of type wgs_set from ENA Portal API.".format(
                count=len(json_data)
            )
        )
        if len(json_data):
        ...
        with gzip.open(unmapped_fasta_file, "rt", encoding="utf-8") as original_f:
            with gzip.open(fasta_file, "wt", encoding="utf-8") as new_f:
                for line in original_f:
                    if line.startswith(">"):
                        contig_name = line[1:]
                        line = ">{analysis_id}.{counter} {contig_name}".format(
